...
Terminal client for XIZ Platform server. Connects over WebSocket (`ws://` or `wss://`) to a running headless server.
...
- For all other commands: a running XIZ Platform headless server with URL and token
