...
import { requestClientConfirmDialog } from '@xiz-platform/server-core/transport'
...
export function registerAuthHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.auth.LOGOUT, async () => {
    try {
      ...
      const configPath = join(process.env.CRAFT_CONFIG_DIR || join(homedir(), '.craft-agent'), 'config.json')
