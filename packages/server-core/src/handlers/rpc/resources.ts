...
import { getCredentialManager, SOURCE_CREDENTIAL_TYPES } from '@xiz-platform/shared/credentials'
...
} from '@xiz-platform/shared/resources'
...
export function registerResourcesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(
    ...
    async (_ctx, workspaceId: string, options: ExportResourcesOptions) => {
      ...
      const { exportResources } = await import('@xiz-platform/shared/resources')
  ...
  server.handle(
    ...
    async (_ctx, workspaceId: string, bundle: ResourceBundle, mode: ResourceImportMode) => {
      ...
      const { importResources } = await import('@xiz-platform/shared/resources')
