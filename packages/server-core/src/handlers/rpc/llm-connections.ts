import { RPC_CHANNELS, type LlmConnectionSetup } from '@xiz-platform/shared/protocol'
import { getLlmConnections, getLlmConnection, addLlmConnection, updateLlmConnection, deleteLlmConnection, getDefaultLlmConnection, setDefaultLlmConnection, touchLlmConnection, isCompatProvider, isAnthropicProvider, getDefaultModelsForConnection, getDefaultModelForConnection, type LlmConnection, type LlmConnectionWithStatus, toBedrockNativeId, deriveBedrockRegionPrefix } from '@xiz-platform/shared/config'
import { getCredentialManager } from '@xiz-platform/shared/credentials'
import { setSetupDeferred } from '@xiz-platform/shared/config/storage'
import {
  resolveSetupTestConnectionHint,
  testBackendConnection,
  validateStoredBackendConnection,
} from '@xiz-platform/shared/agent/backend'
import { getModelRefreshService } from '@xiz-platform/server-core/model-fetchers'
import { parseTestConnectionError, createBuiltInConnection, validateModelList, piAuthProviderDisplayName, validateSetupTestInput, setupTestRequiresApiKey, resolveCustomEndpointSetup } from '@xiz-platform/server-core/domain'
import { getWorkspaceOrThrow, buildBackendHostRuntimeContext } from '@xiz-platform/server-core/handlers'
import { pushTyped, type RpcServer } from '@xiz-platform/server-core/transport'
import type { HandlerDeps } from '../handler-deps'
import { randomUUID } from 'node:crypto'
import { CLIENT_OPEN_EXTERNAL } from '@xiz-platform/server-core/transport'
...
export function registerLlmConnectionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.settings.SETUP_LLM_CONNECTION, async (_ctx, setup: LlmConnectionSetup): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (setup.piAuthProvider && !isCustomEndpointCompat) {
        ...
        // Update connection name to show the actual provider (e.g. "XIZ Platform Backend (Google AI Studio)")
        const providerName = piAuthProviderDisplayName(setup.piAuthProvider)
        if (providerName) {
          updates.name = `XIZ Platform Backend (${providerName})`
        }
  ...
  server.handle(RPC_CHANNELS.settings.TEST_LLM_CONNECTION_SETUP, async (_ctx, params: import('@xiz-platform/shared/protocol').TestLlmConnectionParams): Promise<import('@xiz-platform/shared/protocol').TestLlmConnectionResult> => {
  ...
  server.handle(RPC_CHANNELS.pi.GET_API_KEY_PROVIDERS, async () => {
    const { getPiApiKeyProviders } = await import('@xiz-platform/shared/config')
    return getPiApiKeyProviders()
  })

  server.handle(RPC_CHANNELS.pi.GET_PROVIDER_BASE_URL, async (_ctx, provider: string) => {
    const { getPiProviderBaseUrl } = await import('@xiz-platform/shared/config')
  ...
  server.handle(RPC_CHANNELS.llmConnections.TEST, async (_ctx, slug: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      const { parseValidationError } = await import('@xiz-platform/shared/config')
  ...
  server.handle(RPC_CHANNELS.chatgpt.START_OAUTH, async (ctx, connectionSlug: string): Promise<{
    ...
    const { prepareChatGptOAuth } = await import('@xiz-platform/shared/auth')
  ...
  server.handle(RPC_CHANNELS.chatgpt.COMPLETE_OAUTH, async (ctx, args: {
    ...
    try {
      const { exchangeChatGptTokens } = await import('@xiz-platform/shared/auth')
      const credentialManager = getCredentialManager()
  ...
  server.handle(RPC_CHANNELS.copilot.START_OAUTH, async (ctx, connectionSlug: string): Promise<{
    ...
    try {
      const { loginGitHubCopilot } = await import('@xiz-platform/shared/auth')
      const credentialManager = getCredentialManager()
