...
export function registerProjectsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  async function broadcastChanged(workspaceId: string, workspaceRootPath: string): Promise<void> {
    const { loadWorkspaceProjects } = await import('@xiz-platform/shared/projects')
  ...
  server.handle(RPC_CHANNELS.projects.GET_ONE, async (_ctx, workspaceId: string, projectIdOrSlug: string) => {
    ...
    const { loadProject, loadProjectById } = await import('@xiz-platform/shared/projects')
    return loadProject(workspace.rootPath, projectIdOrSlug)
      ?? loadProjectById(workspace.rootPath, projectIdOrSlug)
  })

  // Create a new project
  server.handle(RPC_CHANNELS.projects.CREATE, async (_ctx, workspaceId: string, input: import('@xiz-platform/shared/projects').CreateProjectInput) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { createProject } = await import('@xiz-platform/shared/projects')
  ...
  server.handle(RPC_CHANNELS.projects.UPDATE, async (
    ...
    workspaceId: string,
    projectSlug: string,
    patch: Partial<Omit<import('@xiz-platform/shared/projects').ProjectConfig, 'id' | 'slug' | 'createdAt'>>,
  ) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { updateProject } = await import('@xiz-platform/shared/projects')
  ...
  server.handle(RPC_CHANNELS.projects.DELETE, async (_ctx, workspaceId: string, projectSlug: string) => {
    ...
    const { loadProject, deleteProject } = await import('@xiz-platform/shared/projects')
    ...
    const { unbindProjectFromSessions } = await import('@xiz-platform/shared/sessions')
    const touched = await unbindProjectFromSessions(workspace.rootPath, project.config.id)
    const { unbindProjectFromPages } = await import('@xiz-platform/shared/pages')
  ...
  server.handle(RPC_CHANNELS.projects.LIST_ASSETS, async (_ctx, workspaceId: string, projectSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) return []
    const { listProjectAssets } = await import('@xiz-platform/shared/projects')
  ...
  server.handle(RPC_CHANNELS.projects.UPLOAD_ASSET, async (
    ...
    workspaceId: string,
    projectSlug: string,
    input: import('@xiz-platform/shared/projects').UploadProjectAssetInput,
  ) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { uploadProjectAsset } = await import('@xiz-platform/shared/projects')
  ...
  server.handle(RPC_CHANNELS.projects.DELETE_ASSET, async (
    ...
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { deleteProjectAsset } = await import('@xiz-platform/shared/projects')
