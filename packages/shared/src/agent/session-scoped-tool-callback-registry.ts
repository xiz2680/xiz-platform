...
import type { AuthRequest } from '@xiz-platform/session-tools-core';
...
export interface SessionScopedToolCallbacks {
  ...
  getSessionInfoFn?: (sessionId?: string) => import('@xiz-platform/session-tools-core').SessionInfo | null;
  /** List sessions in the workspace with pagination. */
  listSessionsFn?: (options?: import('@xiz-platform/session-tools-core').ListSessionsOptions) => import('@xiz-platform/session-tools-core').ListSessionsResult;
  /** List background tasks (running + terminal) for a session from the main-process registry. */
  listBackgroundTasksFn?: (sessionId?: string) => import('@xiz-platform/session-tools-core').BackgroundTaskInfo[];
  /** Resolve label display names to IDs. */
  resolveLabelsFn?: (labels: string[]) => import('@xiz-platform/session-tools-core').ResolvedLabelsResult;
  /** Resolve a status display name to its ID. */
  resolveStatusFn?: (status: string) => import('@xiz-platform/session-tools-core').ResolvedStatusResult;
  /** Send a message to another session (inter-session messaging). Resolves with delivery status. */
  sendAgentMessageFn?: (sessionId: string, message: string, attachments?: Array<{ path: string; name?: string }>) => Promise<import('@xiz-platform/session-tools-core').SendAgentMessageResult>;
  /**
  ...
  /** Create a XIZ Platform Task (board card + task.yaml + orchestrator session) without running it. */
  createTaskFn?: (
    input: import('@xiz-platform/session-tools-core').CreateTaskInput
  ) => Promise<import('@xiz-platform/session-tools-core').CreateTaskResult>;
  /**
   ...
   */
  pages?: import('@xiz-platform/session-tools-core').PagesToolCallbacks;
}
