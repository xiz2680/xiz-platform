#!/usr/bin/env bun
/**
 * Debug script to print the full XIZ Platform system prompt with annotations.
...
printAnnotation('  2. XIZ Platform Environment Marker - version, platform, arch');
...
const systemPromptWithDebug = getSystemPrompt(
  undefined,
  { enabled: true, logFilePath: '~/Library/Logs/@xiz-platform/electron/main.log' },
...
${colors.bold}SDK Configuration:${colors.reset}
  ...
  systemPrompt.append: getSystemPrompt() ${colors.dim}// XIZ Platform additions (static, cacheable)${colors.reset}
...
${colors.bold}Static System Prompt Components:${colors.reset}
  ...
  2. XIZ Platform Environment Marker      ${colors.dim}// Version, platform, arch${colors.reset}
