...
export class WindowManager {
  ...
  private eventSink: ((channel: string, target: import('@xiz-platform/shared/protocol').PushTarget, ...args: any[]) => void) | null = null
   ...
   */
  setRpcEventSink(
    sink: (channel: string, target: import('@xiz-platform/shared/protocol').PushTarget, ...args: any[]) => void,
  ...
  getRpcEventSink(): ((channel: string, target: import('@xiz-platform/shared/protocol').PushTarget, ...args: any[]) => void) | null {
   ...
   *   1 window  → app name ("XIZ Platform") on the lone window
