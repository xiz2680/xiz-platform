...
import type { LlmConnection } from '@xiz-platform/shared/config/llm-connections'
import {
  formatTokenCount,
...
describe('groupConnectionsByProvider', () => {
  ...
  test('places "Anthropic" group before pi groups (display order)', () => {
    ...
    expect(result.map(([k]) => k)).toEqual(['Anthropic', 'XIZ Platform Backend'])
  })
