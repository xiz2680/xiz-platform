...
### Why it's cool
This is the first step toward full programmatic control of Craft Agents. Script your agent workflows, integrate into CI/CD pipelines, or run sessions headlessly on remote machines. Partially addresses [#343](https://github.com/lukilabs/craft-agents-oss/issues/343).
...
### Why it's cool
Deploy Craft Agents on a remote server and connect from anywhere. Run it on your home lab, a cloud VM, or a CI runner. The thin-client mode means the browser UI stays lightweight while all the heavy lifting happens server-side. Partially addresses [#291](https://github.com/lukilabs/craft-agents-oss/issues/291).
...
- **Server-core extraction** — 14 core handlers, session management, services, and domain utilities extracted into `@xiz-platform/server-core` for reuse across Electron and headless
- **Protocol formalization** — channels, DTOs, and event maps extracted to `@xiz-platform/shared/protocol` with wire-format stability tests
