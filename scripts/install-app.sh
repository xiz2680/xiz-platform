...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="XIZ-Platform-x64.AppImage"
...
if [ -z "$filename" ]; then
    filename="XIZ-Platform-${arch}.${ext}"
fi
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.xiz2680.xiz-platform"
    if pgrep -x "XIZ Platform" >/dev/null 2>&1; then
        info "Quitting XIZ Platform..."
        ...
        if pgrep -x "XIZ Platform" >/dev/null 2>&1; then
            ...
            pkill -9 -x "XIZ Platform" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  XIZ Platform has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'XIZ Platform'${NC}"
    echo ""
    ...
    APPIMAGE_INSTALL_PATH="$APP_DIR/XIZ-Platform-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Craft-Agent.*AppImage" >/dev/null 2>&1; then
        info "Stopping XIZ Platform..."
...
#!/bin/bash
# XIZ Platform launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.craft-agent/app/XIZ-Platform-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@craft-agent"
ELECTRON_CACHE_ALT="$HOME/.cache/@craft-agent"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: XIZ Platform not found at $APPIMAGE_PATH"
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/XIZ-Platform-x64.AppImage"
